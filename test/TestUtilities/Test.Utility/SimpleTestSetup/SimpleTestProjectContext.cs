            ...
            get
            {
                var _packageSpec = new PackageSpec(Frameworks
                    .Select(f => new TargetFrameworkInformation() { FrameworkName = f.Framework,
                        Dependencies = f.PackageReferences.Select(e => new LibraryDependency() { LibraryRange = new LibraryRange(e.Id, VersionRange.Parse(e.Version), LibraryDependencyTarget.Package) }
                     ).ToList()
